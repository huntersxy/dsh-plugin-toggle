...
// 列表直接返回 patch 行 id + 版本/描述，无 include: 前缀问题，单一事实源是补丁文件。
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PluginTogglePage(props) {
      ...
      // 重启：两段式确认，避免误触
      var restartState = react.useState({ armed: false, busy: false, message: null });
      var restartInfo = restartState[0];
      var setRestartInfo = restartState[1];
      var doRestart = function () {
        if (restartInfo.busy) return;
        if (!restartInfo.armed) { setRestartInfo({ armed: true, busy: false, message: null }); return; }
        setRestartInfo({ armed: false, busy: true, message: "正在重启 DSH…" });
        props.restart().then(
          function () { setRestartInfo({ armed: false, busy: true, message: "正在重启 DSH，页面即将断开，请稍后刷新" }); },
          function (err) { setRestartInfo({ armed: false, busy: false, message: "重启失败：" + (err && err.message ? err.message : String(err)) }); },
        );
      };
      ...
      var children = [
        react.createElement("p", { key: "note", style: noteStyle },
          "列出每个 profile 中非官方（第三方）插件行。开关写入 profile 的 cordis.patch.yml（disabled 标记），DSH 热重载立即生效。"),
      ];
      ...
      if (status.status === "loading") {
        children.push(react.createElement("p", { key: "loading" }, "加载插件列表…"));
      }
      if (status.status === "ready") {
        var profiles = status.value.profiles || [];
        if (profiles.length === 0) {
          children.push(react.createElement("p", { key: "empty" }, "没有可管理的第三方插件"));
        }
        for (var pi = 0; pi < profiles.length; pi++) {
          (function (profile) {
            ...
            if (!profile.rows || profile.rows.length === 0) {
              children.push(react.createElement("p", { key: "e-" + profile.name, style: { opacity: 0.6 } }, "没有可管理的第三方插件"));
              return;
            }
      ...
      // 重启区块
      var restartSectionStyle = {
        marginTop: 18, paddingTop: 14, borderTop: "1px solid rgba(128,128,128,.25)",
      };
      var restartMsgStyle = { margin: "0 0 8px", opacity: 0.85, color: restartInfo.message && restartInfo.message.indexOf("失败") === 0 ? "#e05353" : undefined };
      children.push(react.createElement("div", { key: "restart", style: restartSectionStyle },
        react.createElement("div", { style: { fontWeight: 600 } }, "DSH 重启"),
        react.createElement("p", { style: noteStyle },
          "重启进程以加载 profile 变更（新增插件、bundle 栈调整等）。页面会短暂断开，完成后刷新即可。"),
        restartInfo.message ? react.createElement("p", { key: "restart-msg", style: restartMsgStyle }, restartInfo.message) : null,
        react.createElement("button", {
          onClick: doRestart,
          style: { border: "1px solid rgba(224,83,83,.55)", background: restartInfo.armed ? "rgba(224,83,83,.22)" : "transparent", color: "#e05353", borderRadius: 6, padding: "6px 14px", cursor: "pointer", fontSize: 12 },
          disabled: restartInfo.busy,
        }, restartInfo.busy ? "重启中…" : (restartInfo.armed ? "再点一次确认重启" : "重启 DSH")),
      ));
    ...
    function apply(ctx) {
      ...
      var injected = function () { return { list: list, setDisabled: setDisabled, restart: restart }; };
