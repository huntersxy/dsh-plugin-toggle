...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PluginTogglePage(props) {
      ...
      // 切换后的警告提示（如：补丁含配置/表达式行 → 热禁用可能不完整）
      ...
      var toggle = function (profile, row) {
        ...
        props.setDisabled(profile, row.id, nextEnabled).then(
          function (res) {
            if (res && res.warnRestart) {
              setNotice("已写入禁用，但该插件补丁含配置/表达式行，热禁用可能不完整——重启进程后完全生效");
            }
            load();
          },
          function (err) { setStatus({ status: "error", message: err && err.message ? err.message : String(err) }); },
      ...
      if (notice) {
        children.push(react.createElement("p", { key: "notice", style: { margin: "0 0 10px", color: "#e8a33d" } }, notice));
      }
